...
def set_angle(angle):
    ...
    if not 0 <= angle <= 180:
        ...
        return
    duty = 2 + (angle / 18)  # Calculate duty cycle for servo
...
# Function to drive straight, compare sensor distances, and adjust the steering
def drive_straight():
    target_distance = 10  # Target distance from the wall in cm
    tolerance = 2  # Distance tolerance for steering adjustments (in cm)
    ...
    try:
        while True:
            ...
            if distance2 <= 20:
                ...
                set_angle(45)  # Go straight (neutral position)
            ...
            sleep(0.1)  # Small delay to avoid overloading the system
...
# Call the function to drive straight
drive_straight()
