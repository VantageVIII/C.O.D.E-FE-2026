...
sensor_b = DistanceSensor(echo=, trigger=)    # Front sensor
...
def perform_sequence():
    ...
    distance1 = sensor_rf.distance * 100 # Right front sensor in cm
    distance2 = sensor_rb.distance * 100 # Right back sensor in cm
    distance3 = sensor_f.distance * 100  # Front sensor in cm
    distance4 = sensor_lf.distance * 100 #Left front sensor in cm
    distance5 = sensor_lb.distance * 100 #Left back sensor in cm
    distance6 = sensor_b.distance * 100 # Back sensor in cm
    ...
    if min_range <= distance3 <= max_range:
        ...
        set_angle(min(angle, 105)) #limit the right angle
        forward(30)  # Increase forward speed for faster response
